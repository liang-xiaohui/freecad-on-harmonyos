...
结论先说：**约 15~20%**，而且失败形态很贵 —— 不是干脆驳回，是**等 2~6 周后被告知"请按品牌化
路径改名"**。依据：
...
| 3 | 2022-10 注册商标的公告写明动机：打击"**在 Windows/Apple 商店卖假 FreeCAD 的人**" | 我们免费、开源、真源码 —— 没有"值得他们破例"的紧迫性 |
| 4 | 一旦给某个移植者发了授权书，各平台的移植者都会来要 | 志愿者理事会最省事的答复永远是"请走品牌化路径，用你自己的名字" |
| 5 | 驳回原文说的是"与『**FreeCAD』应用**的名称相同" | ⚠️ 若平台上真已存在一个叫 FreeCAD 的应用，冲突对象就不是 FPA，**拿到授权书也解不了 1.3**。这条要在手机上实搜一次确认（AGC 搜索页要 JS，命令行查不到） |

发信仍有价值，但价值是**拿一封 FPA 的回信**（留一条"已与上游沟通过"的软证据，并确认他们是否
愿意把鸿蒙移植接纳为官方项目），**不是**拿到能直接交 AGC 的权利文件。
...
| **B** | 拿到 FPA 的授权书 | ⚠️ **成功率约 15~20%**（依据见上一小节） | 保留原名原图标；但失败要等 2~6 周才被告知，沉没成本高，期间还压着改名额度 |
...
10. **处置 AGC 第 2 次驳回：名称/图标与「FreeCAD」冲突**（2026-09-18，详见第 7.6 节）。
   ...
   **结论（2026-09-18 定）**：走 **A 改名 + 重画图标** —— 查证后确认这不是退让，而是 FreeCAD
   官方文档化支持的方式（wiki《品牌化》页明文允许基于 FreeCAD 做应用并换名换图标，只有
   **不改开源**才被禁）；而 **B（向 FPA 要授权书）成功率约 15~20%**，且失败要等 2~6 周。
   B 仍可并行发一封信，但只当"摘桃子"，**不进排期**。
   若走 A，连带改动清单、命名约束与三条必守条件见 7.6；名称一年只能改 2 次，一次做到位。
   信稿（已按"已知品牌化路径"重写问法）：`docs/fpa-brand-permission-request.md`。
   **另需人工确认**：在手机 AGC 里实搜 "FreeCAD"，看平台是否已存在同名应用 ——
   若有，则冲突对象不是 FPA，B 路彻底出局。
