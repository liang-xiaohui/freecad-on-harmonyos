...
- ⚠️ **真机截图天然不是 16:9，要专门处理。**《素材规范》PC/2in1 档写死"**16:9 / 1920×1080**"，
  而本机（2in1）截图是 **2472×1608**（≈1.54，3:2）——**直接缩放会变形、拉伸是审核明确禁止的**
  （"不得重复/模糊/拉伸"）。两条可行路子：① 把应用窗口调成 16:9 的客户区再重新截图（最干净）；
  ② 居中裁切成 16:9 再缩到 1920×1080（会切掉上下桌面/任务栏）。本机**没有任何图像工具**
  （无 Pillow、无 ImageMagick、无 ffmpeg、无 djpeg），所以这一步只能在有工具的机器上做，
  或者手写 JPEG 编解码——不值得。
